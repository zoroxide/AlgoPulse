...
async function runCode({language = "", code = "", input = ""}) {
    ...
    if (compileCodeCommand) {
        await new Promise((resolve, reject) => {
            ...
            compileCode.stderr.on('data', (error) => {
                reject({
                    status: 200,
                    output: 'Compile Time ERROR !', // ta2reeban 3ady (3amelha compilation error wala la2?) 
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(async () => {
            ...
            reject({
                // ha5aleha t return time error fe text soon bs 3shan a3raf enha timeout bas me4 aktar 30 seconds
                status: 408,
        ...
        executeCode.stdin.on('error', (err) => {
            console.log('stdin err', err); // log ba2a W bta3
        });
